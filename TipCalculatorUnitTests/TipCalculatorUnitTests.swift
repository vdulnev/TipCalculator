...
//
//  Created by Viacheslav Dulnev on 20.01.2022.
//
...
class TipCalculatorUnitTests: XCTestCase {
    ...
    func testTipCalculation() throws {
        ...
        let tip = try! tipCalculator.calculate(total: 100, tipPersantage: 0.1)
        XCTAssertEqual(10, tip)
    }
    
    func testNegativeTotal() {
        let tipCalculator = TipCalculator()
        XCTAssertThrowsError(try tipCalculator.calculate(total: -100, tipPersantage: 0.1), "", {error
            in
